...
class _IssueScreenState extends State<IssueScreen> {
  ...
  Future<void> saveIssue() async {
    ...
    try {
      final createRes = await _apiService.post(
        baseUrl: ApiConstants.baseUrl,
        endpoint: ApiConstants.outboundDispatches,
        body: {
          "customerReference": "AUTO-ISSUE-001",
        },
      );

      if (createRes.statusCode != 200) {
        throw Exception('Failed to create outbound dispatch');
      }

      final createData = jsonDecode(createRes.body) as Map<String, dynamic>;
      final dispatchId = createData["outboundDispatchId"];

      if (dispatchId == null) {
        throw Exception('Dispatch ID not returned');
      }

      for (final item in addedItems) {
        final allocateRes = await _apiService.post(
          baseUrl: ApiConstants.baseUrl,
          endpoint: '${ApiConstants.outboundDispatches}/$dispatchId/allocate',
          body: {
            "barcode": item["barcode"],
          },
        );

        if (allocateRes.statusCode != 200) {
          throw Exception('Failed to allocate barcode ${item["barcode"]}');
        }
      }

      final assignRes = await _apiService.post(
        baseUrl: ApiConstants.baseUrl,
        endpoint: '${ApiConstants.outboundDispatches}/$dispatchId/assign',
        body: {
          "vehicleId": "1",
          "tradesPersonId": "1",
        },
